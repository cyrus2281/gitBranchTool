...
__g_help(){
  ...
  echo -e "   s      <id|alias>"
  echo -e "   s      <unregistered-id> [<alias> <note>] \t  When switching to an unregistered branch, you can provide new alias"
  echo -e "\t\t\t\t\t\t\t and optional note to register it at the same time."
...
__g_switch(){
  ...
  if (( $# < 1 )); then
    ...
    echo -e "\tg switch <unregistered-id> [<alias> <note>]"
    return 1
  fi
